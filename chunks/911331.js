...
var o = function (e, t) {
	...
	for (
		var I = n.visualViewport ? n.visualViewport.width : innerWidth,
			m = n.visualViewport ? n.visualViewport.height : innerHeight,
			T = window.scrollX || pageXOffset,
			...
			R = S.bottom,
			C = S.left,
			y = 'start' === o || 'nearest' === o ? v : 'end' === o ? R : v + A / 2,
			D = 'center' === l ? C + N / 2 : 'end' === l ? O : C,
			L = [],
			b = 0;
		b < h.length;
		b++
				...
				(W =
					'start' === l
						...
						: 'center' === l
							...
							: 'end' === l
								? D - G + Z + K
								: s(B, G, w, V, Z + K, D, D + N, N));
			var X = M.scrollLeft,
				$ = M.scrollTop;
			(y += $ - (j = Math.max(0, Math.min($ + j / Q, M.scrollHeight - U / Q + z)))),
				(D += X - (W = Math.max(0, Math.min(X + W / q, M.scrollWidth - w / q + K))));
		}
		L.push({
			el: M,
			top: j,
