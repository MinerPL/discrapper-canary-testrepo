...
function u(e, t, n) {
	...
	var h = o[1],
		...
		m = o[4];
	for (d = 0; d < _; d++)
		(o = e[i >>> 24] ^ h[(a >> 16) & 255] ^ p[(s >> 8) & 255] ^ I[255 & t] ^ r[E]),
			(u = e[a >>> 24] ^ h[(s >> 16) & 255] ^ p[(t >> 8) & 255] ^ I[255 & i] ^ r[E + 1]),
			(c = e[s >>> 24] ^ h[(t >> 16) & 255] ^ p[(i >> 8) & 255] ^ I[255 & a] ^ r[E + 2]),
			(t = e[t >>> 24] ^ h[(i >> 16) & 255] ^ p[(a >> 8) & 255] ^ I[255 & s] ^ r[E + 3]),
			(E += 4),
			(i = o),
			(a = u),
			(s = c);
	for (d = 0; 4 > d; d++)
		(f[n ? 3 & -d : d] =
			(m[i >>> 24] << 24) ^ (m[(a >> 16) & 255] << 16) ^ (m[(s >> 8) & 255] << 8) ^ m[255 & t] ^ r[E++]),
			(o = i),
			(i = a),
			(a = s),
			(s = t),
		...
		16 > n
			...
			: ((r = t[(n + 1) & 15]),
				...
				(r = t[15 & n] =
					...
					0)),
			(r =
				r + f + ((d >>> 6) ^ (d >>> 11) ^ (d >>> 25) ^ (d << 26) ^ (d << 21) ^ (d << 7)) + (E ^ (d & (_ ^ E))) + s[n]),
			(f = E),
			(E = _),
			(_ = d),
			(d = (c + r) | 0),
...
(l.cipher.aes = function (e) {
	...
	for (this.b = [(r = e.slice(0)), (i = [])], e = t; e < 4 * t + 28; e++)
		(n = r[e - 1]),
			(0 == e % t || (8 === t && 4 == e % t)) &&
				...
				0 == e % t && ((n = (n << 8) ^ (n >>> 24) ^ (o << 24)), (o = (o << 1) ^ (283 * (o >> 7))))),
			(r[e] = r[e - t] ^ n);
	for (t = 0; e; t++, e--)
		(n = r[3 & t ? e : e - 4]),
			(i[t] =
	...
	(l.cipher.aes.prototype = {
		...
		O: function () {
			...
			for (t = n = 0; !u[t]; t ^= r || 1, n = _[n] || 1)
				for (
					a = ((a = n ^ (n << 1) ^ (n << 2) ^ (n << 3) ^ (n << 4)) >> 8) ^ (255 & a) ^ 99,
						u[t] = a,
						c[a] = t,
						s = (16843009 * (i = d[(e = d[(r = d[t])])])) ^ (65537 * e) ^ (257 * r) ^ (16843008 * t),
						i = (257 * d[a]) ^ (16843008 * a),
						e = 0;
					4 > e;
					e++
				)
	...
	(l.bitArray = {
		...
		$: function (e, t, n, r) {
			...
			return (
				...
				(e = l.bitArray.getPartial(i)),
				r.push(l.bitArray.partial((t + e) & 31, 32 < t + e ? n : r.pop(), 1)),
				r
			);
		},
		i: function (e, t) {
	...
	(l.codec.base32 = {
		...
		fromBits: function (e, t, n) {
			...
			for (n && (o = l.codec.base32.X), n = 0; a.length * r < c; )
				(a += o.charAt((u ^ (e[n] >>> s)) >>> i)),
					s < r ? ((u = e[n] << (r - s)), (s += i), n++) : ((u <<= r), (s -= r));
			for (; 7 & a.length && !t; ) a += '=';
			return a;
		},
		toBits: function (e, t) {
	...
	(l.codec.base64 = {
		...
		fromBits: function (e, t, n) {
			...
			for (n && (a = a.substr(0, 62) + '-_'), n = 0; 6 * r.length < o; )
				(r += a.charAt((s ^ (e[n] >>> i)) >>> 26)),
					6 > i ? ((s = e[n] << (6 - i)), (i += 26), n++) : ((s <<= 6), (i -= 6));
			for (; 3 & r.length && !t; ) r += '=';
			return r;
		},
		toBits: function (e, t) {
	...
	(l.mode.ccm = {
		...
		encrypt: function (e, t, n, r, i) {
			...
			return (
				a < 15 - u && (a = 15 - u),
				(n = o.clamp(n, 8 * (15 - a))),
				(t = l.mode.ccm.V(e, t, n, r, i, a)),
				(s = l.mode.ccm.C(e, s, n, t, i, a)),
				o.concat(s.data, s.tag)
			);
		},
		decrypt: function (e, t, n, r, i) {
			...
			if (
				(t < 15 - s && (t = 15 - s),
				(n = a.clamp(n, 8 * (15 - t))),
				(u = l.mode.ccm.C(e, u, n, c, i, t)),
				(e = l.mode.ccm.V(e, u.data, n, r, i, t)),
				!a.equal(u.tag, e))
			)
		...
		na: function (e, t, n, r, i, a) {
			var s = [],
				...
				u = o.i;
			if (
				((r = [o.partial(8, (t.length ? 64 : 0) | ((r - 2) << 2) | (a - 1))]),
				(r = o.concat(r, n)),
				(r[3] |= i),
				(r = e.encrypt(r)),
				t.length)
				...
				for (
					65279 >= (n = o.bitLength(t) / 8)
						? (s = [o.partial(16, n)])
						: 4294967295 >= n && (s = o.concat([o.partial(16, 65534)], [n])),
						s = o.concat(s, t),
						t = 0;
					t < s.length;
					t += 4
				)
					r = e.encrypt(u(r, s.slice(t, t + 4).concat([0, 0, 0])));
			return r;
		},
		V: function (e, t, n, r, i, a) {
			...
			for (n = l.mode.ccm.na(e, r, n, i, s.bitLength(t) / 8, a), r = 0; r < t.length; r += 4)
				n = e.encrypt(o(n, t.slice(r, r + 4).concat([0, 0, 0])));
	...
	(l.mode.ocb2 = {
		...
		encrypt: function (e, t, n, r, i, a) {
			...
			return (
				...
				(_ = u.clamp(c(_.concat([0, 0, 0]), s), t)),
				(d = c(d, c(_.concat([0, 0, 0]), s))),
				(d = e.encrypt(c(d, c(n, o(n))))),
				r.length && (d = c(d, a ? r : l.mode.ocb2.pmac(e, r))),
				E.concat(u.concat(_, u.clamp(d, i)))
			);
		},
		decrypt: function (e, t, n, r, i, a) {
			...
			if (
				((o = f - 32 * n),
				(s = e.encrypt(d(E, [0, 0, 0, o]))),
				(s = d(s, c.clamp(t.slice(n), o).concat([0, 0, 0]))),
				(_ = d(_, s)),
				(_ = e.encrypt(d(_, d(E, u(E))))),
				r.length && (_ = d(_, a ? r : l.mode.ocb2.pmac(e, r))),
				!c.equal(c.clamp(_, i), c.bitSlice(t, f)))
			)
		...
		pmac: function (e, t) {
			...
			return (
				(n = t.slice(n)),
				128 > i.bitLength(n) && ((o = a(o, r(o))), (n = i.concat(n, [-2147483648, 0, 0, 0]))),
				(s = a(s, n)),
				e.encrypt(a(r(a(o, r(o))), s))
			);
		},
		S: function (e) {
			return [
	...
	(l.mode.gcm = {
		...
		decrypt: function (e, t, n, r, i) {
			...
			if (
				((r = r || []),
				(i = i || 128) <= o ? ((t = s.bitSlice(a, o - i)), (a = s.bitSlice(a, 0, o - i))) : ((t = a), (a = [])),
				(e = l.mode.gcm.C(!1, e, a, r, n, i)),
				!s.equal(e.tag, t))
			)
		...
		ka: function (e, t) {
			...
			for (n = 0, i = [0, 0, 0, 0], a = t.slice(0); 128 > n; n++) {
				for (
					(r = 0 != (e[Math.floor(n / 32)] & (1 << (31 - (n % 32))))) && (i = o(i, a)), s = 0 != (1 & a[3]), r = 3;
					0 < r;
					r--
				)
		...
		C: function (e, t, n, r, i, a) {
			...
			for (
				_ = n.length,
					...
					s = t.encrypt([0, 0, 0, 0]),
					96 === o
						? ((i = i.slice(0)), (i = h.concat(i, [1])))
						: ((i = l.mode.gcm.j(s, [0, 0, 0, 0], i)),
							(i = l.mode.gcm.j(s, i, [0, 0, Math.floor(o / 4294967296), 4294967295 & o]))),
					o = l.mode.gcm.j(s, [0, 0, 0, 0], r),
					d = i.slice(0),
					r = o.slice(0),
	...
	(l.misc.hmac = function (e, t) {
		...
		for (this.w = [new t(), new t()], e.length > i && (e = t.hash(e)), n = 0; n < i; n++)
	...
	(l.misc.pbkdf2 = function (e, t, n, r, i) {
		...
		'string' == typeof e && (e = l.codec.utf8String.toBits(e)),
			'string' == typeof t && (t = l.codec.utf8String.toBits(t)),
			(e = new (i = i || l.misc.hmac)(e));
		var a,
			s,
			o,
			u,
		...
		for (u = 1; 32 * c.length < (r || 1); u++) {
			for (s = 1, i = a = e.encrypt(d.concat(t, [u])); s < n; s++)
				for (a = e.encrypt(a), o = 0; o < a.length; o++) i[o] ^= a[o];
			c = c.concat(i);
		}
	...
	(l.prng.prototype = {
		randomWords: function (e, t) {
			...
			if (n & this.J) {
				...
				for (
					a = 0;
					a < this.c.length &&
					((i = i.concat(this.c[a].finalize())), (s += this.m[a]), (this.m[a] = 0), n || !(this.P & (1 << a)));
					a++
				);
				for (
					this.P >= 1 << this.c.length && (this.c.push(new l.hash.sha256()), this.m.push(0)),
						this.f -= s,
						s > this.o && (this.o = s),
						this.P++,
						this.b = l.hash.sha256.hash(this.b.concat(i)),
						this.L = new l.cipher.aes(this.b),
						n = 0;
		...
		addEntropy: function (e, t, n) {
			...
			(this.m[s] += t),
				(this.f += t),
				o === this.u &&
					(this.isReady() !== this.u && d('seeded', Math.max(this.o, this.f)), d('progress', this.getProgress()));
		},
		isReady: function (e) {
			return (
...
i: try {
	...
	if (s && i.randomBytes)
		(r = i.randomBytes(128)),
			...
			l.random.addEntropy(r, 1024, "crypto['randomBytes']");
...
(l.json = {
	...
	ja: function (e, t, n, r) {
		...
		if (
			(a.g(s, n),
			(n = s.adata),
			'string' == typeof s.salt && (s.salt = l.codec.base64.toBits(s.salt)),
			'string' == typeof s.iv && (s.iv = l.codec.base64.toBits(s.iv)),
			!l.mode[s.mode] ||
				!l.cipher[s.cipher] ||
		...
		return (
			'string' == typeof e
				? ((e = (i = l.misc.cachedPbkdf2(e, s)).key.slice(0, s.ks / 32)), (s.salt = i.salt))
				: l.ecc &&
					e instanceof l.ecc.elGamal.publicKey &&
					((i = e.kem()), (s.kemtag = i.tag), (e = i.key.slice(0, s.ks / 32))),
			'string' == typeof t && (t = l.codec.utf8String.toBits(t)),
			'string' == typeof n && (s.adata = n = l.codec.utf8String.toBits(n)),
			(i = new l.cipher[s.cipher](e)),
			a.g(r, s),
			(r.key = e),
			(s.ct =
				'ccm' === s.mode && l.arrayBuffer && l.arrayBuffer.ccm && t instanceof ArrayBuffer
					? l.arrayBuffer.ccm.encrypt(i, t, s.iv, n, s.ts)
					: l.mode[s.mode].encrypt(i, t, s.iv, n, s.ts)),
			s
		);
	},
	...
	ia: function (e, t, n, r) {
		...
		var i,
			...
			s = l.json;
		if (
			((i = (t = s.g(s.g(s.g({}, s.defaults), t), n, !0)).adata),
			'string' == typeof t.salt && (t.salt = l.codec.base64.toBits(t.salt)),
			'string' == typeof t.iv && (t.iv = l.codec.base64.toBits(t.iv)),
			!l.mode[t.mode] ||
				!l.cipher[t.cipher] ||
		...
		return (
			'string' == typeof e
				? ((e = (a = l.misc.cachedPbkdf2(e, t)).key.slice(0, t.ks / 32)), (t.salt = a.salt))
				: l.ecc &&
					e instanceof l.ecc.elGamal.secretKey &&
					(e = e.unkem(l.codec.base64.toBits(t.kemtag)).slice(0, t.ks / 32)),
			'string' == typeof i && (i = l.codec.utf8String.toBits(i)),
			(a = new l.cipher[t.cipher](e)),
			(i =
				'ccm' === t.mode && l.arrayBuffer && l.arrayBuffer.ccm && t.ct instanceof ArrayBuffer
					? l.arrayBuffer.ccm.decrypt(a, t.ct, t.iv, t.tag, i, t.ts)
					: l.mode[t.mode].decrypt(a, t.ct, t.iv, i, t.ts)),
			s.g(r, t),
			(r.key = e),
			1 === n.raw ? i : l.codec.utf8String.fromBits(i)
		);
	},
	...
	decode: function (e) {
		...
		for (t = 0; t < e.length; t++) {
			if (
				!(n = e[t].match(/^\s*(?:(["']?)([a-z][a-z0-9]*)\1)\s*:\s*(?:(-?\d+)|"([a-z0-9+\/%*_.@=\-]*)"|(true|false))$/i))
			)
				throw new l.exception.invalid("json decode: this isn't json!");
			null != n[3]
				? (r[n[2]] = parseInt(n[3], 10))
				: null != n[4]
					? (r[n[2]] = n[2].match(/^(ct|adata|salt|iv)$/) ? l.codec.base64.toBits(n[4]) : unescape(n[4]))
	...
	(l.misc.cachedPbkdf2 = function (e, t) {
		...
		return (
			(n = (t = t || {}).iter || 1000),
			(n = (r = r[e] = r[e] || {})[n] =
				r[n] || { firstSalt: t.salt && t.salt.length ? t.salt.slice(0) : l.random.randomWords(2, 0) }),
