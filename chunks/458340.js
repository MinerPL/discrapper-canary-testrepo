...
function s(e, t, n, r, i) {
	for (
		...
		(a = u[E >>> 24] ^ c[(f >>> 16) & 255] ^ d[(h >>> 8) & 255] ^ _[255 & p] ^ t[I++]),
			(s = u[f >>> 24] ^ c[(h >>> 16) & 255] ^ d[(p >>> 8) & 255] ^ _[255 & E] ^ t[I++]),
			(o = u[h >>> 24] ^ c[(p >>> 16) & 255] ^ d[(E >>> 8) & 255] ^ _[255 & f] ^ t[I++]),
			(l = u[p >>> 24] ^ c[(E >>> 16) & 255] ^ d[(f >>> 8) & 255] ^ _[255 & h] ^ t[I++]),
			(E = a),
			(f = s),
			(h = o),
			(p = l);
	return (
		(a = ((r[E >>> 24] << 24) | (r[(f >>> 16) & 255] << 16) | (r[(h >>> 8) & 255] << 8) | r[255 & p]) ^ t[I++]),
		(s = ((r[f >>> 24] << 24) | (r[(h >>> 16) & 255] << 16) | (r[(p >>> 8) & 255] << 8) | r[255 & E]) ^ t[I++]),
		(o = ((r[h >>> 24] << 24) | (r[(p >>> 16) & 255] << 16) | (r[(E >>> 8) & 255] << 8) | r[255 & f]) ^ t[I++]),
		(l = ((r[p >>> 24] << 24) | (r[(E >>> 16) & 255] << 16) | (r[(f >>> 8) & 255] << 8) | r[255 & h]) ^ t[I++]),
		[(a >>>= 0), (s >>>= 0), (o >>>= 0), (l >>>= 0)]
	);
}
	...
	l = (function () {
		...
		for (var n = [], r = [], i = [[], [], [], []], a = [[], [], [], []], s = 0, o = 0, l = 0; l < 256; ++l) {
			...
			(i[0][s] = (E << 24) | (E >>> 8)),
				...
				(i[2][s] = (E << 8) | (E >>> 24)),
				(i[3][s] = E),
				(E = (16843009 * _) ^ (65537 * d) ^ (257 * c) ^ (16843008 * s)),
				(a[0][u] = (E << 24) | (E >>> 8)),
				(a[1][u] = (E << 16) | (E >>> 16)),
				(a[2][u] = (E << 8) | (E >>> 24)),
				(a[3][u] = E),
				0 === s ? (s = o = 1) : ((s = c ^ e[e[e[_ ^ c]]]), (o ^= e[e[o]]));
		}
		return {
			SBOX: n,
			INV_SBOX: r,
	...
	(u.prototype._reset = function () {
		...
		for (var u = [], c = 0; c < r; c++) {
			...
			c < 4 || d <= 4
				? (u[c] = _)
				: (u[c] =
						l.INV_SUB_MIX[0][l.SBOX[_ >>> 24]] ^
						l.INV_SUB_MIX[1][l.SBOX[(_ >>> 16) & 255]] ^
						l.INV_SUB_MIX[2][l.SBOX[(_ >>> 8) & 255]] ^
						l.INV_SUB_MIX[3][l.SBOX[255 & _]]);
		}
