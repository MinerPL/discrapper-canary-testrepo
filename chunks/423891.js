...
r(444675),
			...
			if ((m && ((d = m[0]), (v = m[1]), (g = m[2])), 8 == l))
				for (var p = 0; p < i; p++) {
					...
					(s[y] = t[b]),
						...
						(s[y + 2] = t[b + 2]),
						(s[y + 3] = 255),
						-1 != d && t[b] == d && t[b + 1] == v && t[b + 2] == g && (s[y + 3] = 0);
				}
			if (16 == l)
				for (var p = 0; p < i; p++) {
					var y = p << 2,
					...
					(s[y] = t[b]),
						...
						(s[y + 2] = t[b + 4]),
						(s[y + 3] = 255),
						-1 != d && h(t, b) == d && h(t, b + 2) == v && h(t, b + 4) == g && (s[y + 3] = 0);
				}
		} else if (3 == c) {
			var E = n.tabs.PLTE,
				T = n.tabs.tRNS,
		...
		for (; r < e.length; ) {
			...
			if (((r += 4), 'IHDR' == d)) o.decode._IHDR(e, r, s);
				...
				3 == s.ctype
					...
					: 0 == s.ctype
						? (s.tabs[d] = i(e, r))
						: 2 == s.ctype && (s.tabs[d] = [i(e, r), i(e, r + 2), i(e, r + 4)]);
		...
		for (
			...
			for (var _ = 0, A = l[m]; A < n; ) {
				for (var I = h[m], S = (c + _ * T) << 3; I < r; ) {
					...
					if (i >= 8) {
						for (var L = A * s + I * a, P = 0; P < a; P++) u[L + P] = t[(S >> 3) + P];
					}
					(S += i), (I += v);
				}
				_++, (A += d);
	...
	(o.encode = function (t, e, r, n, i, a) {
		...
		for (
			var s = new Uint8Array(t[0].byteLength * t.length + 100), u = [137, 80, 78, 71, 13, 10, 26, 10], c = 0;
			c < 8;
			c++
		)
			s[c] = u[c];
		...
		for (var I = 0, S = 0; S < v.frames.length; S++) {
			...
			g &&
				...
				d(s, (l += 4), 'fcTL'),
				p(s, (l += 4), I++),
				p(s, (l += 4), H.rect.width),
				p(s, (l += 4), H.rect.height),
				p(s, (l += 4), H.rect.x),
				p(s, (l += 4), H.rect.y),
				m(s, (l += 4), i[S]),
				m(s, (l += 2), 1000),
			...
			if (0 != h && !T) {
				for (var w = i || 1 == h || 2 == A[A.length - 2].dispose ? 1 : 2, R = 0, M = 1000000000, C = 0; C < w; C++) {
					...
					V < M &&
						((M = V),
						(R = C),
						-1 == G ? ((H = L = 0), (P = B = 1)) : ((H = U), (L = F), (P = G - U + 1), (B = k - F + 1)));
				}
				var O = new Uint8Array(t[h - 1 - R]);
				1 == R && (A[A.length - 1].dispose = 2);
				var Z = new Uint8Array(P * B * 4);
				new Uint32Array(Z.buffer),
					o._copyTile(O, e, r, Z, P, B, -H, -L, 0),
					o._copyTile(I, e, r, Z, P, B, -H, -L, 3)
						? (o._copyTile(I, e, r, Z, P, B, -H, -L, 2), (N = 1))
						: (o._copyTile(I, e, r, Z, P, B, -H, -L, 0), (N = 0)),
	...
	(o.quantize = function (t, e, r) {
		...
		for (var T = 0; T < n.length; T++) {
			for (
				...
				for (
					var H = A[a] * (1 / 255), L = A[a + 1] * (1 / 255), P = A[a + 2] * (1 / 255), B = A[a + 3] * (1 / 255), N = p;
					N.left;
	...
	(o.quantize.estats = function (t) {
		...
		if (0 != n)
			for (
				var d = 0;
				d < 10 &&
				((f = h.multVec(l, f)), (m = Math.sqrt(h.dot(f, f))), (f = h.sml(1 / m, f)), !(1e-9 > Math.abs(m - p)));
				d++
			) {
				p = m;
			}
