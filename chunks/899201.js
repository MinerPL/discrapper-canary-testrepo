...
function V(e) {
	...
	let K = (0, d.Wu)([M.Z, w.Z], () =>
		M.Z.getInviteSuggestionRows().filter(
			(e) => e.type === D.bm.FRIEND || e.type === D.bm.DM || w.Z.can(L.Plq.ATTACH_FILES, e.item)
		)
	);
	n.useEffect(() => {
		(0, E.x)({
				...
				f.Z.uploadFiles({
					...
					parsedMessage:
						null != p
							? C.ZP.parse(
									r,
									F.Z.Messages.EMBEDDED_ACTIVITIES_SHARE_MOMENT_FROM_APP.format({
			...
			try {
				await Promise.all(s),
					null != p &&
						(0, u.showToast)(
							(0, u.createToast)(
								F.Z.Messages.EMBEDDED_ACTIVITIES_SHARE_MOMENT_FROM_APP.format({ applicationName: p.name }),
								u.ToastType.SUCCESS
							)
						);
			} catch (e) {
...
function W(e) {
	let { applicationId: a, mediaUrl: t, activitySessionId: s } = e,
		[l, r] = n.useState(!1),
		o = (0, d.e7)([x.default], () => x.default.getCurrentUser());
	async function c() {
