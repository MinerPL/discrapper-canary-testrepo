...
class _ extends r.Z {
	...
	handleRingUpdate() {
		let e = a.Z.getCalls().filter(
				(e) => e.ringing.length > 0 && l.Z.getCurrentClientVoiceChannelId(null) === e.channelId
			),
			t = l.Z.getCurrentClientVoiceChannelId(null);
		(null != t && u.ZP.countVoiceStatesForChannel(t) >= 2) ||
		!(e.length > 0) ||
		s.Z.isSoundDisabled('call_calling') ||
		o.Z.disableSounds
			? d.stop()
			: d.loop();
	}
	constructor(...e) {
		...
		super(...e),
			...
			(r = new Map()
				...
				.set(s.Z, this.handleRingUpdate)
				.set(o.Z, this.handleRingUpdate)
				.set(l.Z, this.handleRingUpdate)
				.set(i.Z, this.handleSoundpackUpdate)),
			n in t
